...
public class AttachmentSo extends DragonBaseServiceObject {
	...
	public void processAggregation(String fieldName) throws DragonBusinessException{
		if(fieldName.equals("originFileUrl")){
			this.setOriginFileUrl(this.attachmentService.getFileUrlString(this.id, "origin",DragonConstant.CONTENT_DEPOSITION_INLINE));
		}else if(fieldName.equals("mediumFileUrl")){
			this.setMediumFileUrl(this.attachmentService.getFileUrlString(this.id, "medium",DragonConstant.CONTENT_DEPOSITION_INLINE));
		}else if(fieldName.equals("thumbFileUrl")){
			this.setThumbFileUrl(this.attachmentService.getFileUrlString(this.id, "thumb",DragonConstant.CONTENT_DEPOSITION_INLINE));
		}
	}
